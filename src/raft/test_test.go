...
func TestReElection2A(t *testing.T) {
	...
	fmt.Printf("断掉leader%d\n", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader. and the old leader
	// should switch to follower.
	fmt.Printf("重连leader%d\n", leader1)
	cfg.connect(leader1)
...
// snapcommon 是测试快照功能的通用函数
// t: 测试句柄，用于报告测试结果
// name: 测试名称，用于标识测试
// disconnect: 是否在测试中断开服务器连接
// reliable: 网络是否可靠
// crash: 是否在测试中模拟服务器崩溃
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	iters := 30                                     // 测试迭代次数
	servers := 3                                    // 服务器数量
	cfg := make_config(t, servers, !reliable, true) // 创建测试配置
	defer cfg.cleanup()                             // 测试结束后清理资源

	cfg.begin(name) // 开始测试
	fmt.Printf("开始测试：%s\n", name)

	cfg.one(rand.Int()%101, servers, true) // 发送一个随机命令以初始化Raft状态
	leader1 := cfg.checkOneLeader()        // 检查并获取当前的领导者
	fmt.Printf("初始领导者为：服务器 %d\n", leader1)

	for i := 0; i < iters; i++ {
		victim := (leader1 + 1) % servers // 计算下一个要断开连接或崩溃的服务器
		sender := leader1                 // 默认情况下，领导者发送命令
		if i%3 == 1 {
			sender = (leader1 + 1) % servers // 每三次迭代改变一次发送者
			victim = leader1                 // 并将领导者设置为受害者
			//fmt.Printf("新sender为：服务器 %d\n", sender)
			//fmt.Printf("此轮断链服务器为：服务器 %d\n", leader1)
		}

		// 根据disconnect和crash标志断开连接或模拟崩溃
		if disconnect {
			cfg.disconnect(victim)
			fmt.Printf("断开服务器 %d 的连接，测试两个节点的一致性情况\n", victim)
			cfg.one(rand.Int()%101, servers-1, true) // 在少一个服务器的情况下发送命令
		}
		if crash {
			cfg.crash1(victim)
			fmt.Printf("崩溃服务器 %d\n", victim)
		...
		for j := 0; j < nn; j++ {
			cfg.rafts[sender].Start(rand.Int() % 101)
		}
		fmt.Printf("迭代次数%d服务器 %d 发送了 %d 次操作以尝试触发快照\n", i, sender, nn)

		// 确保所有追随者都已经追上了最新的状态
		if disconnect == false && crash == false {
			cfg.one(101, servers, true)
			fmt.Printf("服务器 %d 发送了 101 尝试达成共识\n", sender)
		} else {
			cfg.one(101, servers-1, true)
		}
		...
		// 处理连接和崩溃后的恢复
		if disconnect {
			cfg.connect(victim) // 重新连接服务器
			fmt.Printf("重新连接disconnect服务器 %d\n", victim)
			//time.Sleep(RaftElectionTimeout)
			cfg.one(rand.Int()%101, servers, true) // 发送命令以确保状态一致
			leader1 = cfg.checkOneLeader()         // 检查领导者是否发生了变化
			fmt.Printf("新的领导者为：服务器 %d\n", leader1)
		}
		if crash {
			cfg.start1(victim, cfg.applierSnap) // 重启服务器
			cfg.connect(victim)                 // 重新连接服务器
			fmt.Printf("重启并重新连接crash服务器 %d\n", victim)
			//time.Sleep(RaftElectionTimeout)
			cfg.one(rand.Int()%101, servers, true) // 发送命令以确保状态一致
			leader1 = cfg.checkOneLeader()         // 检查领导者是否发生了变化
			fmt.Printf("新的领导者为：服务器 %d\n", leader1)
...
func TestSnapshotAllCrash2D(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		...
		index1 := cfg.one(rand.Int()%101, servers, true)
		DPrintf("迭代 %d，第一个 index1 = %v\n", i+1, index1)
		...
		for j := 0; j < servers; j++ {
			...
			cfg.connect(j)
			DPrintf("重新启动并连接服务器 %d\n", j)
		}

		index2 := cfg.one(rand.Int()%101, servers, true)
		DPrintf("迭代 %d，第二个 index2 = %v\n", i+1, index2)

		if index2 < index1+1 {
			t.Fatalf("index 减小了，从 %v 减小到 %v\n", index1, index2)
		}
	}
	cfg.end()
}
