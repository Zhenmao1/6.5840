...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	if snapshot == nil {
		DPrintf("服务器 %d 收到了一个空的快照\n", i)
		log.Fatalf("nil snapshot")
		return "nil snapshot"
	}
	DPrintf("服务器 %d 开始处理快照，快照大小为 %d，收到的index%d\n", i, len(snapshot), index)
	...
	if index != -1 && index != lastIncludedIndex {
		...
		DPrintf("服务器 %d 的快照不匹配，期望的快照索引为 %d, 但实际为 %d\n", i, index, lastIncludedIndex)
		return err
	}
	...
	DPrintf("服务器 %d 成功处理了快照，最后包含的索引为 %d\n", i, lastIncludedIndex)
	return ""
}
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			if !retry {
				nd, cmd1 := cfg.nCommitted(index)
				cfg.t.Fatalf(" 函数one cmd(%v),在状态机的下标，一致的数量%d,检测的index%d,,cmd1%v", cmd, nd, index, cmd1)
			}
		} else {
	...
	if !cfg.checkFinished() {
		nd, cmd1 := cfg.nCommitted(index)
		cfg.t.Fatalf(" one(%v) failed to reach agreement，一致的数量%d,检测的index%d,cmd1%v", cmd, nd, index, cmd1)
		//cfg.t.Fatalf(" 函数one(%v) failed to reach agreement", cmd)
	}
	return -1
}
