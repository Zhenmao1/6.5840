...
// 基于通道的RPC，用于6.5840实验。
//
// 模拟一个可以丢失请求、丢失回复、延迟消息，并完全断开特定主机的网络。
//
// 我们将使用原始的labrpc.go来测试您的代码进行评分。
// 因此，虽然您可以修改此代码以帮助调试，但请在提交之前针对原始代码进行测试。
//
// 改编自Go net/rpc/server.go。
//
// 发送labgob编码的值，以确保RPC不包含对程序对象的引用。
//
// net := MakeNetwork() -- 持有网络、客户端和服务器。
// end := net.MakeEnd(endname) -- 创建一个客户端端点，用于与一个服务器通信。
// net.AddServer(servername, server) -- 将一个命名服务器添加到网络中。
// net.DeleteServer(servername) -- 消除命名服务器。
...
// 并且回复是有效的。
// Call()返回false如果网络丢失了请求或回复
// 或服务器宕机。
// 在同一个ClientEnd上同时进行多个Call()是可以的。
// 并发调用Call()可能以不同的顺序传递到服务器，
// 因为网络可能重新排列消息。
// Call()保证返回（可能有延迟）*除非*
// 服务器端的处理函数不返回。
// 服务器RPC处理函数必须将其args和reply参数声明为指针，
// 以使它们的类型与Call()的参数的类型完全匹配。
//
// srv := MakeServer()
